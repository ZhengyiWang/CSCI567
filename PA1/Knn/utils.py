...
class HyperparameterTuner:
    ...
    def tuning_without_scaling(self, distance_funcs, x_train, y_train, x_val, y_val):
        ...
        self.best_distance_function=list(distance_funcs.values())[index[0]]
    ...
    def tuning_with_scaling(self, distance_funcs, scaling_classes, x_train, y_train, x_val, y_val):
        ...
        for k in range(1, min(len(x_train),30), 2):
            f1_score_records.append([])
            for scaler_name,scaler in listscaling_classes.items():
                tmp=[]
                fun=scaler()
                for dis_func_name,distance_function in distance_funcs.items():
                    model=KNN(k,distance_function)
        ...
        self.best_distance_function=list(distance_funcs.values())[index[1]]
        self.best_k=index[2]*2+1
        self.best_scaler=list(scaling_classes.values())[index[0]]
        self.best_model=KNN(self.best_k,self.best_distance_function)
        self.best_model.train(x_train,y_train)
