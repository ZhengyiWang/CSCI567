...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        zero_matrix=np.zeros((np.size(X,0),np.size(X,1)))
        forward_output=np.maximum(X,zero_matrix)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        dh_du=1-np.power(np.tanh(X),2)
        
        backward_output=np.multiply(grad,dh_du)
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    momentum[module_name+"_"+key]=_alpha*momentum[module_name+"_"+key]-learning_rate*g
